...
def run(img):
    ...
    if not start_pick_up:
        target_color_lab = (lab_data[target_color]['min'][:3], lab_data[target_color]['max'][:3])
        # mask the colors we want
        frame_mask = cv2.inRange(frame, *target_color_lab)
        ...
        biggest_contour, biggest_contour_area: float = max(
            zip(contours, areas), key=lambda c, area: area)
        if biggest_contour_area and biggest_contour_area > 10:
